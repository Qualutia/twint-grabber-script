import twint, numpy, nltk, datetime, os, json, io, re
...
# if argv[4] == 'true' :
    # open("./src/data/tweets_temp.json", "w", encoding="utf-8").close
    # open("./src/data/tweets.json", "w", encoding="utf-8").close
...
file.write("\t]\n}")
    ...
    # # Formats the Tweets
    # for y in range(len(user_tweets)):

    #     tweet_str = str(user_tweets['tweet'].values[y])
    #     tweet = json.dumps(tweet_str).replace('"', ' ').replace('\\', ' ')

    #     tweet_mentions_temp = re.findall("(^|[^@\w])@(\w{1,15})", tweet)
    #     tweet_mentions = []

    #     for mention in tweet_mentions_temp:
    #         tweet_mentions.append(mention[1])


    #     reply_to_str = user_tweets['reply_to'].values[y]
    #     reply_to = json.dumps(reply_to_str)


    #     reply_to_list = []

    #     for x in range(len(user_tweets['reply_to'].values[y])):

    #         target = user_tweets['reply_to'].values[y][x]

    #         reply_to_list.append(target['screen_name'])



    #     file.write( "\t\t\t" + "{" + '"id"' + ':' + str(user_tweets['id'].values[y]) + ',' +'"tweet"' + ':' + '"' + str(tweet) + '"' + ',' + '"mention"' + ':' + str(json.dumps(tweet_mentions)) + ',' + '"reply_to"' + ':' + str(json.dumps(reply_to_list)))


    #     if len(user_tweets) == y + 1:
    #         file.write("}\n")
